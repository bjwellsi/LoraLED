    ...
    if(radioOps.messageWaiting()){
        ...
        if(!nextMessage.targetId.idMatches(TID, UID)){
            ...
            if(it != inboundLedger.end()) {
                ...
                if(found.expectsAck && (found.responseCode == ComDef::AckResponseCode::SUCCESS 
                    || found.responseCode == ComDef::AckResponseCode::ERROR
                    || found.responseCode == ComDef::AckResponseCode::CANCELED
                    || found.responseCode == ComDef::AckResponseCode::TIMEOUT))
                {
                    ...
                    if(!(found.sentAck == nullptr)){
                        ...
                        //technically this method overwrites the responsecode but I'm overwriting it with itself so sue me
...
bool MessageTransport::messageWaiting(){
...
RadioDTO::Message* MessageTransport::sendMessage(RadioDTO::Message message, int resendCount, int timeout){
    //assign the sequenceid and return a reference to the packet in the outbound ledger
